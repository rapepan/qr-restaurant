...
import { ShoppingCart, Bell, Receipt, Search, Loader2, X } from 'lucide-react';
...
type ToastMsg = { message: string; type: 'success' | 'error' | 'info' } | null;
            ...
            {menu.map((cat) => (
              <button
                ...
                className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all active:scale-95 ${
                  activeCat === cat.id ? 'bg-brand-600 text-white shadow-md' : 'bg-gray-100 text-gray-600'
                }`}
              >
